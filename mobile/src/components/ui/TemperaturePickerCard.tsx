...
const TemperaturePickerCard: React.FC<Props> = ({ theme }) => {
	...
	// THE FIX: Changed from NodeJS.Timeout to ReturnType<typeof setInterval> for React Native compatibility
	...
	// CLEANUP: Prevents memory leaks if the component unmounts while holding the button
	useEffect(() => {
		return () => stopHoldAction();
	}, []);

	// --- Core Math Handlers (Using prev state for interval safety) ---
	...
	const startHoldingDecrease = () => {
		...
		holdIntervalRef.current = setInterval(decrementValue, 50); // 100ms firing speed
	};

	const startHoldingIncrease = () => {
		if (holdIntervalRef.current) clearInterval(holdIntervalRef.current);
		holdIntervalRef.current = setInterval(incrementValue, 50); // 100ms firing speed
	};
	...
	return (
		...
		>
			...
			<View style={styles.contentContainer}>
				{/* DECREASE BUTTON */}
				<Pressable
					onPress={decrementValue} // Fires once on a normal tap
					onLongPress={startHoldingDecrease} // Fires continuously if held
					onPressOut={stopHoldAction} // Stops firing when you let go or slide finger off
					delayLongPress={300} // Waits 300ms before entering "rapid fire" mode
...
const styles = StyleSheet.create({
	...
	// THE FIX: Removed maxWidth. Used minWidth and padding to allow dynamic growth
	focalBox: {
		minWidth: 140, // Keeps it perfectly square for 2 digits
		paddingHorizontal: 10, // Allows it to expand gracefully for 3 digits
		height: 165,
		borderRadius: 24,
		alignItems: "center",
		justifyContent: "center",
	...
	// THE FIX: Swapped `width` for `minWidth` so the wrapper expands with the text
	inputWrapper: {
		height: 160,
		minWidth: 120,
		justifyContent: "center",
	...
	// THE FIX: Swapped `width` for `minWidth` so Android doesn't scroll the invisible text
	mainNumberInput: {
		fontFamily: "aera_tallcompressed",
		fontSize: 140,
		lineHeight: 140,
		height: 140,
		minWidth: 120, // Lets 100 stretch the box natively
